...
public class Species {
	...
	public double estimate_average() {
		...
		for (int i=0;i< organisms.size();i++) {
			...
			if (curorg.time_alive >= GASpecies.TIME_ALIVE_MINIMUM) {
	...
	public Organism reproduce_one(int generation, Population pop,
		...
		// Now elig_orgs is guaranteed to contain either an ordered list of
		// mature orgs or all the orgs (Ken)
		// We may also want to check to see if we are getting pools of >1
		// organism (to make sure our survival_thresh is sensible) (Ken)

		// Only choose from among the top ranked orgs
		poolsize = (int) ((elig_orgs.size() - 1) * GASpecies.SURVIVAL_THRESHOLD);
		
		// First, decide whether to mate or mutate
		// If there is only one organism in the pool, then always mutate
		if ((generator.nextFloat() < GASpecies.MUTATE_ONLY_PROBABILITY)
				|| poolsize == 0) {
		...
		// Otherwise we should mate
			...
			// Choose random dad
			if (generator.nextFloat() < GASpecies.INTERSPECIES_MATE_RATE) {
				// Mate within Species
				...
				while ((randspecies == this) && (giveup < 5)) {
					...
					//randmult = GaussRand(generator) / 4;
				...
				// Make dad be a champ from the random species
			...
			if ((generator.nextFloat() > GASpecies.MATE_ONLY_PROB)
