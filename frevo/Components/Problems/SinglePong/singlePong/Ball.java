...
/**
 * Represents a ball in the single Pong game. 
 * 
...
public class Ball extends FieldObject {
	...
	public Ball(SinglePongParameters parameters, int number)
	{
		super(parameters);
		this.number = number;
	 ...
	 */
	public void update(double dt, SinglePongState state)
	{
		...
		if ((pos.dy > 0) && (pos.y > this.maxY)) {
			...
			pos.y = this.minY;
		    pos.dy = -pos.dy;
		} else if ((pos.x > this.maxX) && (pos.dx > 0))	{
			pos.x = this.maxX;
			pos.dx = -pos.dx;
		}
		
		for (int i=0; i<state.getTeam().length; i++)
		{
			Paddle paddle =  state.getTeam()[i];
	...
	public void reset() {
		...
		if (this.number == 0)
		{
			this.setPosition(this.minX, y);
		    this.setDirection(this.speed, this.speed);
		}
		else if (this.number == 1)
		{
			this.setPosition(this.maxX, y);
		    this.setDirection(-this.speed, this.speed);
		}
		...
		{
			this.setPosition(minX + halfDistance/2, y);
		    this.setDirection(this.speed, this.speed);
		}
		else if (this.number == 5) 
		{
			this.setPosition(minX + 3*halfDistance/2, y);
		    this.setDirection(-this.speed, this.speed);
		}		
