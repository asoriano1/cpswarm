...
/**
 * Storage of parameters for Pong game 
 * 
 * @author Sergii Zhevzhyk
 */
public class PongParameters {
	...
	private static int controllingPositionProbes = 100;
	...
	// calculate the distance to another teammate only from one side
	private boolean teammateFromOneSide = false;
	...
	public void initialize(Hashtable<String, XMLFieldEntry> parameters, Random random){
		...
		XMLFieldEntry teammateFromOneSideEntry = parameters.get("teammateFromOneSide");
		if (teammateFromOneSideEntry != null) {
			this.teammateFromOneSide = Boolean.parseBoolean(teammateFromOneSideEntry
					.getValue());
		}
	...
	public static int getControllingPositionProbes() {
		return controllingPositionProbes;
	}
	...
	public boolean isTeammateFromOneSide() {
		return teammateFromOneSide;
	}
